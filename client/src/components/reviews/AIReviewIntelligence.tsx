...
const IconMap: Record<string, any> = {
  Waves, Sun, Utensils, Mountain, Baby, Users, Car, CloudSun, CalendarDays, Wallet, CircleAlert, Sparkles, Star, Target, Heart, BarChart3, Bot, Camera
};
...
function SentimentAnalysisCard({ data }: { data: any }) {
  const shouldReduceMotion = useReducedMotion();
...
function LoveAndConcernsCard({ data }: { data: any }) {
  const shouldReduceMotion = useReducedMotion();
...
function ReviewsPickedForYouCard({ data }: { data: any }) {
  const shouldReduceMotion = useReducedMotion();
...
export default function AIReviewIntelligence() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/reviews/insights`)
      .then(res => res.json())
      .then(res => {
  ...
  if (loading || !data) {
    return (
      <section className="bg-[#F7F7F2] py-5 sm:py-6 lg:py-7 min-h-[400px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#087F5B] border-t-transparent"></div>
          <p className="text-[#66736D] font-medium text-sm">Generating AI Insights...</p>
        </div>
      </section>
    );
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <SentimentAnalysisCard data={data} />
          <LoveAndConcernsCard data={data} />
          <ReviewsPickedForYouCard data={data} />
        </div>
      </div>
    </section>
  );
