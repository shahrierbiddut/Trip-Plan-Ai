  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      if (data.reply) {
        ...
        setAnswer(data.error || "Sorry, I couldn't get an answer right now.");
      }
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        {/* Suggestions or Answer */}

        <div className="mt-3 space-y-1.5 flex-1 max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-4">
              <div className="h-4 w-4 animate-spin rounded-full border-2 border-[#F4B942] border-t-transparent"></div>
              <p className="text-[9px] text-[#F4B942] mt-2">Thinking...</p>
            </div>
          ) : answer ? (
            <div className="text-[9px] leading-relaxed text-white/90 bg-white/5 p-2.5 rounded-lg border border-white/10">
              {answer}
              <button 
                onClick={() => { setAnswer(""); setQuestion(""); }}
                className="mt-2 text-[#F4A62A] font-semibold hover:underline flex items-center gap-1"
              >
        ...
        >
          ...
          <motion.button
            ...
            whileHover={
              shouldReduceMotion || loading || !question.trim()
                ? undefined
                : {
                    scale: 1.08,
                    rotate: -4,
            ...
            className={`
              ...
              text-[#063A2F]
              transition-colors
              \${(loading || !question.trim()) ? "opacity-50 cursor-not-allowed" : "hover:bg-[#F8B94C]"}
            `}
          >
            <Send size={11} />
          </motion.button>
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/reviews`)
      ...
      .then((data) => {
        if (data.success && data.data && Array.isArray(data.data.reviews)) {
          const formatted = data.data.reviews.map((r: any) => ({
            id: r._id,
            name: r.name || "Traveler",
            avatar: r.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(r.name || "Traveler")}&background=F4A934&color=073D31`,
            location: r.location || "Bangladesh",
            rating: Number(r.rating) || 5,
            title: r.title || "Amazing Experience",
            text: r.reviewText || r.text || "",
            destination: r.destination || "",
            tripType: r.tripType || "",
            date: r.date ? new Date(r.date).toLocaleDateString("en-US", { month: "short", year: "numeric" }) : "Recently",
            helpful: r.helpful || 0,
            verified: r.verified || false,
            photos: Array.isArray(r.images) ? r.images.map((img: any) => img.data || img) : [],
          }));
          setFetchedReviews(formatted);
        }
        setLoading(false);
      })
      .catch((err) => {
